...
class WebhookHandler(BaseHTTPRequestHandler):
    ...
    def _parse_and_display(self, body: bytes) -> dict:
        ...
        try:
            ...
            parsed: Any = json.loads(decoded_stripped) if decoded_stripped else {}
            # JSON ルートが文字列で、その中身がさらに JSON（二重エンコード）のケース
            ...
            if isinstance(parsed, dict):
                ...
                result = {"raw": decoded, "parsed_non_object": str(parsed)[:500]}
        except json.JSONDecodeError:
            result = {"raw": decoded}
    ...
    def _run_job(self, payload: dict) -> None:
        ...
        if not isinstance(payload, dict):
            _write_log(f"[Job] スキップ: ペイロードがオブジェクトではありません ({type(payload).__name__})\n")
            return
        try:
    ...
    def _execute_mt5_order(self, payload: dict) -> None:
        ...
        try:
            ...
            from MQL5.webhook_parse import WebhookOrderSkip, parse_webhook_for_mt5
        except ImportError as e:
            _write_log(f"[MQL5] スキップ: MQL5 モジュールのインポートに失敗 - {e}\n")
            return

        mt5_config = config.get("mt5", {})
        parsed = parse_webhook_for_mt5(payload, mt5_config)
        if isinstance(parsed, WebhookOrderSkip):
            _write_log(f"[MQL5] スキップ: {parsed.reason}\n")
            return
