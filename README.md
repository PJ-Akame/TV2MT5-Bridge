...
## 概要
<img width="1276" height="656" alt="image" src="https://github.com/user-attachments/assets/1a66b7dd-fdb0-4e13-963d-e4c601a2f5c3" />
...
**アラート本文の JSON 形（Webhook の POST ボディと同じオブジェクト）**

`SMCExecutionSignal.pine` は次のキーだけを送ります（`volume` は送らず、省略時は `config.mt5.volume`）。
...
*注文しない（SMA クロスはあるが条件不一致 → `No trade`）*
...
*任意でロットを上書きする場合（Pine 未送信でもサーバ側で解釈可）*
...
### mapping.json（銘柄マップ・任意）

TradingView / Webhook の `symbol` と MetaTrader 5 上の**正式銘柄名**が一致しない場合に、`config/mapping.json` で **Webhook 側表記 → MT5 名**を指定します（`config.json` とは別ファイル）。

```powershell
copy config\mapping.json.example config\mapping.json
```

- 形式: トップレベル JSON オブジェクト **`"キー": "MT5で使う名前"`**。同一ターゲットに対し **`XAUUSD` と `FX:XAUUSD` のように複数キー**を並べられます。
- キー名が **`_` で始まる**項目は無視します（メモ用に使えます）。
- ファイルが無い、またはオブジェクトが空のときは、従来どおり **`EXCHANGE:` プレフィックスの除去**のみ行い、マップは使いません。
- 適用箇所: `parse_webhook_for_mt5`（Webhook 経路）および `order.execute_order`（CLI 経路）。
...
│   ├── mapping.json.example   # 銘柄マップテンプレート
│   └── mapping.json           # 実際のマップ（任意・Git に含めない）
